...
function Get-IndexPaths {
    ...
    [pscustomobject]@{
        Json = Join-Path $WorkspaceRoot "indice-repositorios-root-$Usuario.json"
        Markdown = Join-Path $WorkspaceRoot "indice-repositorios-root-$Usuario.md"
    }
}
...
$PeerUsuario = if ($UsuarioMaquina -eq "jf") { "jz" } else { "jf" }
$indexPaths = Get-IndexPaths -WorkspaceRoot $WorkspaceRoot -Usuario $UsuarioMaquina
...
$jsonObj = [pscustomobject]@{
    ...
    machine_user = $UsuarioMaquina
    peer_user = $PeerUsuario
    ticket_id = $TicketId
...
if (-not $JsonOnly) {
    ...
    $lines += "- Usuario da maquina: $UsuarioMaquina"
    $lines += "- Usuario observado: $PeerUsuario"
...
[pscustomobject]@{
    JsonPath = $jsonPath
    MarkdownPath = $indexPaths.Markdown
    UsuarioMaquina = $UsuarioMaquina
    PeerUsuario = $PeerUsuario
